...
const LENDERS = [
  { name: 'First Citizens Bank', file: 'firstcitizens.svg' },
  { name: 'Republic Bank', file: 'republicbank.svg' },
  { name: 'JMMB', file: 'jmmb.svg' },
  { name: 'Sagicor', file: 'sagicor.svg' },
  { name: 'Scotiabank TT', file: 'scotiabank.svg' },
  { name: 'Caribbean Union Bank', file: 'caribbeanunion.svg' },
  { name: "People's Credit Union", file: 'peoplescu.svg' },
  { name: 'Teachers Credit Union', file: 'teacherscu.svg' },
  { name: 'Police Credit Union', file: 'policecu.svg' },
  { name: 'CDB', file: 'cdb.svg' },
  { name: 'TT Mortgage Finance', file: 'ttmf.svg' },
  { name: 'GATE Programme', file: 'gate.svg' },
];
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-xl font-bold text-slate-900 tracking-tight">Lifeguage</span>
          </div>
          <a
            href="https://wa.me/14155238886"
            className="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold py-2 px-5 rounded-lg transition-colors shadow-sm"
          >
      ...
      <section className="pt-16 pb-12 px-6 text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-lg text-slate-600 mb-8 max-w-lg mx-auto leading-relaxed">
            No credit check. No bank visit. Just answer a few quick questions and we&apos;ll show you your loan eligibility with real Trinidad & Tobago lenders.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-4">
            <Link href="https://wa.me/14155238886" className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white font-semibold py-3.5 px-8 rounded-xl transition-colors shadow-lg shadow-green-200 text-base">
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
              </svg>
              Start on WhatsApp — Free
            </Link>
            <Link href="/wizard" className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-white hover:bg-slate-50 text-slate-700 font-semibold py-3.5 px-8 rounded-xl transition-colors shadow-lg border border-slate-200 text-base">
              <MonitorPlay className="w-5 h-5" />
              Try Web Wizard — Also Free
            </Link>
          </div>
          <p className="text-sm text-slate-500">Takes about 3 minutes · No account needed · 100% free</p>

          {/* Social proof */}
          <p className="mt-5 text-sm text-slate-500">
            Trusted by borrowers across Trinidad & Tobago
          </p>
        </div>
      </section>

      {/* ── Scrolling Lender Logos ─────────────────────── */}
      <section className="pb-14 overflow-hidden" aria-label="Lender partners">
        <div className="max-w-5xl mx-auto px-6 mb-4">
          <p className="text-center text-xs font-semibold text-slate-400 uppercase tracking-widest">Lenders we work with</p>
        </div>
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {[
              ...
              { n: '3', title: 'See real Trinidad & Tobago lenders', desc: 'Compare rates from First Citizens, Republic Bank, JMMB, credit unions and more.' },
            ].map(({ n, title, desc }) => (
      ...
      <section className="px-6 pb-16">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {[
              { title: 'Your score out of 100', desc: 'Know exactly where you stand before approaching a lender.' },
              { title: 'Estimated loan amount', desc: 'See how much you could borrow, from $5K up to $500K.' },
              { title: 'Trinidad & Tobago lenders', desc: 'Real banks and credit unions with real rates — click to chat on WhatsApp.' },
              { title: 'Ways to improve your score', desc: 'Get tips on what to fix before applying for a better rate.' },
            ].map(({ title, desc }) => (
