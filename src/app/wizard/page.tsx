...
export default function WizardPage() {
  ...
  const handleNext = async () => {
    ...
    if (step < adaptiveSteps) {
      ...
      try {
        ...
        // Manually set result via a trick — we'll show it as step adaptiveSteps+1
        setStep(999); // special results step
        setAssessing(false);
        // Store result temporarily via a workaround — use state wrapper
        (state as { result?: WizardResult }).result = result;
      } catch {
        setAssessing(false);
      }
    }
  ...
  const currentResult = (state as { result?: WizardResult }).result;
      ...
      <main className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
            ...
            <p className="text-slate-500">Hi {state.name || 'there'}, here's what we found based on your profile</p>
          </motion.div>
